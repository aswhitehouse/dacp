...
class MultiStepAgent(dacp.Agent):
    ...
    def _continue_workflow(self, message, state):
        ...
        if current_step == 1:
            ...
            state["data"]["requirements"] = message.get("ai_response", "Standard project requirements")
...
def demo_multi_step_workflow():
    ...
    response = orchestrator.send_message("workflow-agent", {
    ...
    print("\n📊 Workflow Status:")
    status = orchestrator.send_message("workflow-agent", {
...
def demo_research_workflow():
    ...
    response = orchestrator.send_message("research-agent", {
...
def demo_conversation_history():
    ...
    messages = [
        {"task": "analyze_project", "project_name": "Test Project", "workflow_id": "hist_001"},
    ...
    for i, entry in enumerate(history, 1):
        print(f"  Entry {i}: {entry['agent_name']} - {entry['message'].get('task')} -> {entry['response'].get('workflow_status', 'N/A')} ({entry['duration']:.3f}s)")
