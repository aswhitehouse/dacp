...
def demo_llm_routing():
    ...
    response = orchestrator.send_message("client-agent", {
...
def demo_tool_routing():
    ...
    response = orchestrator.send_message("tool-client", {
        "task": "create_report",
...
def demo_multi_agent_routing():
    ...
    for agent_name in agents:
        ...
        response = orchestrator.send_message(agent_name, {
            "task": "analyze_with_ai",
            "data": f"Task routed to {agent_name} via DACP"
        })
        print(f"🔄 Response from {agent_name}: {response.get('response', response.get('error'))[:100]}...")
    ...
    for agent_name, response in responses.items():
        print(f"  📤 {agent_name}: {response.get('tool_result', {}).get('result', {}).get('message', 'No response')}")
...
def demo_direct_api_usage():
    ...
    result = dacp.execute_tool("file_writer", {
...
def demo_client_tool_registration():
    ...
    response = orchestrator.send_message("custom-client", {
        "task": "custom_tool",
