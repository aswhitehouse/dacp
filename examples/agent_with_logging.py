    ...
    def handle_message(self, message):
        ...
        if task == "greet":
            ...
            return {
                "tool_request": {
                    "name": "file_writer",
                    "args": {"path": "./notes/note.txt", "content": content},
                }
            }
...
def main():
    ...
    response = orchestrator.send_message(
        "simple-agent",
        {"task": "write_note", "content": "This is a note written by an agent!"},
    )
    print(f"Response: {response}")

    print("\n" + "=" * 50)
    print("📤 Requesting intelligence call...")
    response = orchestrator.send_message("simple-agent", {"task": "use_intelligence"})
    print(f"Response: {response}")
