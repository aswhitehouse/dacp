...
def demo_basic_logging():
    ...
    response = orchestrator.send_message("demo-agent", {"task": "write_file"})
    print(f"Response: {response}")
...
def demo_debug_logging():
    ...
    response = orchestrator.send_message("debug-agent", {"task": "write_file"})
    print(f"Response: {response}")
...
def demo_error_logging():
    ...
    response = orchestrator.send_message("error-agent", {"task": "error"})
    print(f"Response: {response}")

    # Try to send message to non-existent agent
    print("\n📤 Sending message to non-existent agent...")
    response = orchestrator.send_message("non-existent", {"task": "anything"})
    print(f"Response: {response}")
...
def demo_intelligence_logging():
    ...
    try:
        dacp.invoke_intelligence("Hello", {"engine": "openai", "model": "gpt-4"})
    except Exception as e:
        print(f"Expected error: {e}")
...
def demo_broadcast_logging():
    ...
    responses = orchestrator.broadcast_message({"task": "greet", "name": "Everyone"})
