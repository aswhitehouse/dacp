...
def _generate_logging_setup(logging_config: Dict[str, Any]) -> str:
    ...
    if env_overrides:
        ...
        if 'format_style' in env_overrides:
            setup_lines.append(f'        format_style = os.getenv("{env_overrides["format_style"]}", "{format_style}")')
        if 'log_file' in env_overrides:
            setup_lines.append(f'        log_file = os.getenv("{env_overrides["log_file"]}", {repr(log_file)})')
